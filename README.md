...
- **注意**: iOS は Apple Developer Program 登録などの工数がかかることから、現時点では通知未対応です。Android（Chrome）およびデスクトップブラウザ（Chrome、Edge、Firefox など）で動作します

### PWA 対応

- **ホーム画面に追加してアプリのように起動可能**: Web App Manifest を設定し、インストールできる PWA として提供
