...
export default function EditHabitModal({
  ...
  useEffect(() => {
    if (isOpen && habit) {
      ...
      setNotificationEnabled(habit.notification?.enabled || false);
      setNotificationTime(habit.notification?.reminderTime || "");
    }
  }, [isOpen, habit]);
    ...
    if (notificationPermission === "default") {
      try {
        ...
        if (result === "granted") {
          ...
          // 拒否された場合はOFFのまま
          console.log("[EditHabitModal] 通知が拒否されました");
          alert(
            "通知が許可されませんでした。ブラウザの設定を確認してください。"
          );
        } else if (result === "default") {
          // "default"の場合は、ユーザーがダイアログで操作していない可能性
          // Edgeブラウザでは、通知許可ダイアログが表示されない場合があります
          console.log("[EditHabitModal] 通知許可の状態がdefaultのままです");
          alert(
            "通知を有効にするには、ブラウザの設定から通知を許可してください。\n\n" +
              "手順：\n" +
              "1. アドレスバーの🔔アイコン（または🔒アイコン）をクリック\n" +
              "2. 「通知」を「許可」に変更\n" +
              "3. ページを再読み込みしてから、再度チェックボックスをオンにしてください"
          );
          // チェックボックスはOFFのまま
        }
      } catch (error) {
